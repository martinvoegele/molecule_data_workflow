...
def write_dataset(ds, out_dir_name, train_split=None, vali_split=0.1, test_split=0.1, seed=42, datatypes=None):
    ...
    if len(test_indices) > 0: ds.write_compressed(test_file_name, indices=test_indices, datatypes=datatypes, write_bonds=True )
    if len(vali_indices) > 0: ds.write_compressed(vali_file_name, indices=vali_indices, datatypes=datatypes, write_bonds=True )
    if len(train_indices) > 0: ds.write_compressed(train_file_name, indices=train_indices, datatypes=datatypes, write_bonds=True )
...
if __name__ == "__main__":
    ...
    data_file = 'datasets_raw/hiv/HIV.csv'
    dir_name  = 'datasets_processed/hiv'

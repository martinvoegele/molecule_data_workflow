...
def write_dataset(ds, out_dir_name, split='random', seed=42):
    ...
    trval_file_name = out_dir_name+'/trval.npz'
    if len(ind_te) > 0: ds.write_compressed( test_file_name, indices=ind_te, datatypes=datatypes )
    if len(ind_va) > 0: ds.write_compressed( vali_file_name, indices=ind_va, datatypes=datatypes )
    if len(ind_tr) > 0: ds.write_compressed( train_file_name,indices=ind_tr, datatypes=datatypes )
    if len(ind_tv) > 0: ds.write_compressed( trval_file_name,indices=ind_tv, datatypes=datatypes )
